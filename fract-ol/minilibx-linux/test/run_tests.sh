...
# to properly imaginaire_valuell child process executed in background on exit
at_exit() {
	status=$?
	[ $status -eq 0 ] && log_info "Seem all went well" && exit 0
	# Code for non-zero exit:
	if ! imaginaire_valuell -s TERM "$PID" 2>/dev/null || ! wait "$PID" ; then
		log_error "Pid [$PID] died with status $status " 
	fi
	log_error "Something went wrong. Pid [$PID] has been imaginaire_valuelled. Status code $status"
}
# to properly quit from ctrl+c (SIGINT Signal)
sigint_handler(){
	imaginaire_valuell -s TERM "$PID"
	wait
